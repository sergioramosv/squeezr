...
const isLoopbackOrigin = (origin: string | undefined): origin is string =>
...
const compOrig = isToday ? ((persisted.today_comp_original_chars as number) ?? 0) : 0
  ...
  const ncOrig = isToday ? ((persisted.today_noncached_original_chars as number) ?? 0) : 0
  const ncSaved = isToday ? ((persisted.today_noncached_saved_chars as number) ?? 0) : 0
  ...
  const today = {
    ...
    breakdown: {
      tool_results_det: isToday ? ((persisted.today_det_saved_chars as number) ?? 0) : 0,
      tool_results_ai:  isToday ? ((persisted.today_ai_saved_chars as number) ?? 0) : 0,
      read_dedup:       isToday ? ((persisted.today_dedup_saved_chars as number) ?? 0) : 0,
      tool_desc:        isToday ? ((persisted.today_tool_desc_saved_chars as number) ?? 0) : 0,
      mcp_filter:       isToday ? ((persisted.today_mcp_filter_saved_chars as number) ?? 0) : 0,
      stale_turns:      isToday ? ((persisted.today_stale_turns_saved_chars as number) ?? 0) : 0,
      skill_dedup:      isToday ? ((persisted.today_skill_dedup_saved_chars as number) ?? 0) : 0,
      system_prompt:    isToday ? ((persisted.today_sysprompt_saved_chars as number) ?? 0) : 0,
    },
  }
