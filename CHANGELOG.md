...
- **Dashboard**: la card "Session Cache" desglosa ahora los Expands en **todo** (bloque entero) vs **parcial** (un segmento `id~i`). Nuevo contador `expand_partial` persistido y expuesto en `/squeezr/stats` (`expand.partial` / `expand.whole`). `recordExpand(found, partial)` detecta el parcial por el `~` del id en las 3 rutas (MCP endpoint + no-streaming Anthropic/OpenAI).
