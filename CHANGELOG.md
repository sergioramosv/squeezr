...
- Tests: **2 nuevos** en `expand.test.ts` — directive lidera con el nombre namespaced cuando esa es la tool registrada, y con el nombre plano cuando solo esa existe. Suite **575/575 verde** (+4 skipped), typecheck limpio.
## [1.99.8] - 2026-09-15
### Added — IP de bind configurable para todos los puertos de Squeezr (`squeezr ip`)
- **`config.ts`**: nuevo `Config.host` (`[proxy].host` / `SQUEEZR_HOST`), default `'127.0.0.1'` — antes el bind era un literal hardcodeado, imposible de cambiar sin tocar el código.
- **`index.ts`**: `httpServer.listen(PORT, '127.0.0.1', ...)` → `httpServer.listen(PORT, config.host, ...)`. Si `config.host` no es loopback, se imprime un warning explícito al arrancar (el proxy guarda las API keys del usuario y expone endpoints de control `/squeezr/*` cuyo único blindaje frente a peticiones no-browser es hoy el bind loopback — ver el middleware CORS/CSRF de `server.ts`).
- **`codexMitm.ts`**: el listener MITM de Codex (`mitmServer.listen`) sigue el mismo `config.host` — mismo default, mismo warning.
- **`desktopProxy.ts`** deliberadamente NO tocado: sus dos listeners deben seguir en `127.0.0.1` porque el truco de redirección vía hosts-file para Claude/Codex Desktop depende de que `api.anthropic.com` resuelva a loopback.
- **`bin/squeezr.js`**: nuevo comando `squeezr ip [address]` — sin argumento muestra el host actual; con argumento lo valida (`net.isIP` o `localhost`), lo persiste en **`~/.squeezr/squeezr.toml`** (la ubicación correcta de overrides de usuario, no el `squeezr.toml` empaquetado que usa `squeezr ports`) y reinicia el daemon. Avisa explícitamente al pasar a un bind no-loopback.
- `squeezr setup` no necesitó cambios: al no escribir `[proxy].host`, el default de `Config` (`127.0.0.1`) ya cubre "localhost salvo que lo cambies tú explícitamente".
- Tests: **6 nuevos** (`config.test.ts`: default/toml/env override de `host`; `serverBinding.test.ts` actualizado para verificar `config.host` en el bind en vez del literal, y el default real de `Config`). Suite **577/577 verde**, typecheck limpio.
## [1.99.6] - 2026-09-10
...
- **Dashboard**: dos cards separadas **Expand · whole** vs **Expand · partial** (contador `expand_partial` persistido, expuesto en `/squeezr/stats` como `expand.partial` / `expand.whole`; `recordExpand(found, partial)` detecta el parcial por el `~` del id en las 3 rutas). Y la tool/directiva `squeezr_expand` ahora explican los DOS expands (todo `id` / parcial `id~i`) para que Claude los use.
- **Dashboard — todo en inglés**: eliminado el spanglish (sección AI Compression de Settings, tooltips del hero, labels). El dashboard queda consistente en inglés.
...
- Refactor: `buildToolsHtml` separado de `renderTools` para reutilización (igual que buildModelHtml/buildClientHtml).
### Notes
- Las tarjetas del Overview siguen mostrando la sesión actual del proxy (en vivo). El histórico completo está en Savings.
## [1.60.0] - 2026-06-04
